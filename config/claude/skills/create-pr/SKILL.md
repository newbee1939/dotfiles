...
5. **push するたびに PR の description を最新化する**（`gh pr edit`）。
   - 実装が「やったこと」から増えた/変わったら、該当の箇条書きを追記・修正する。
   - スコープ外にしていたことを今回やった場合は「あとでやること」から削除する。
   - **push 前に必ず自問する**: 「diff は変わったが description は変えていないままではないか?」
